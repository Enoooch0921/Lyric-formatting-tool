...
可拖入最多 50 MB 的 `.pptx` 檔案。工具會在瀏覽器本機讀取投影片文字，透過頁碼重置、單行歌名頁與背景變化辨識歌曲，再將完整重複或局部重複的歌詞整理成段落。第一個段落優先視為主歌；副歌只會從後續重複段落判斷，並依照演唱順序保守辨識副歌前段（Pre Chorus）與後段橋段（Bridge）。證據不足時會使用主歌編號，不會強制猜測段落名稱。檔案不會上傳到伺服器。
